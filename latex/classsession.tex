\hypertarget{classsession}{}\section{session Class Reference}
\label{classsession}\index{session@{session}}


session module\textquotesingle{}s high class  


Inheritance diagram for session\+:\begin{figure}[H]
\begin{center}
\leavevmode
\includegraphics[height=3.660131cm]{classsession}
\end{center}
\end{figure}
\subsection*{Public Member Functions}
\begin{DoxyCompactItemize}
\item 
\hyperlink{classsession_a96fce5e6e2f2f0504cb1b9b4906ba379}{session} ()
\item 
\hyperlink{classsession_af6b13ec50160d63aca04f689e5cac9dd}{module\+Install} ()
\begin{DoxyCompactList}\small\item\em Additional tasks required to accomplish during the installation. \end{DoxyCompactList}\item 
\hyperlink{classsession_a30f6cc95b00bd35757aa882abce7ac34}{check\+Update} ()
\begin{DoxyCompactList}\small\item\em A method to check if the installation has been successful. \end{DoxyCompactList}\item 
\hyperlink{classsession_ad1eb77a85d7765620658071e49d16cff}{module\+Update} ()
\begin{DoxyCompactList}\small\item\em Execute update. \end{DoxyCompactList}\item 
\hyperlink{classsession_a97462709868f87d6cb050956c7b7da6d}{un\+Serialize\+Session} (\$\hyperlink{ckeditor_2js_2xe__interface_8js_a4fb96abdf073a439bca5e051c333b35d}{val})
\begin{DoxyCompactList}\small\item\em session string decode \end{DoxyCompactList}\item 
\hyperlink{classsession_aea7dba13f8bc60c916840c66b2bc28ba}{serialize\+Session} (\$\hyperlink{document__category_8js_a94b4f25375ac17cb917012e05bf514a7}{data})
\begin{DoxyCompactList}\small\item\em session string encode \end{DoxyCompactList}\item 
\hyperlink{classsession_a08f5190f8867dc2c0d5c23435b236f15}{recompile\+Cache} ()
\begin{DoxyCompactList}\small\item\em Re-\/generate the cache file. \end{DoxyCompactList}\end{DoxyCompactItemize}
\subsection*{Public Attributes}
\begin{DoxyCompactItemize}
\item 
\hyperlink{classsession_af73b8d0d1528237504be7b85f05c97ed}{\$lifetime} = 18000
\item 
\hyperlink{classsession_aae3391920555707a71e14e15f2929825}{\$session\+\_\+started} = false
\end{DoxyCompactItemize}


\subsection{Detailed Description}
session module\textquotesingle{}s high class 

\begin{DoxyAuthor}{Author}
N\+A\+V\+ER (\href{mailto:developers@xpressengine.com}{\tt developers@xpressengine.\+com}) 
\end{DoxyAuthor}
\begin{DoxyVersion}{Version}
0.\+1
\end{DoxyVersion}
The session management class 

Definition at line 11 of file session.\+class.\+php.



\subsection{Member Function Documentation}
\index{session@{session}!check\+Update@{check\+Update}}
\index{check\+Update@{check\+Update}!session@{session}}
\subsubsection[{\texorpdfstring{check\+Update()}{checkUpdate()}}]{\setlength{\rightskip}{0pt plus 5cm}session\+::check\+Update (
\begin{DoxyParamCaption}
{}
\end{DoxyParamCaption}
)}\hypertarget{classsession_a30f6cc95b00bd35757aa882abce7ac34}{}\label{classsession_a30f6cc95b00bd35757aa882abce7ac34}


A method to check if the installation has been successful. 



Definition at line 35 of file session.\+class.\+php.

\index{session@{session}!module\+Install@{module\+Install}}
\index{module\+Install@{module\+Install}!session@{session}}
\subsubsection[{\texorpdfstring{module\+Install()}{moduleInstall()}}]{\setlength{\rightskip}{0pt plus 5cm}session\+::module\+Install (
\begin{DoxyParamCaption}
{}
\end{DoxyParamCaption}
)}\hypertarget{classsession_af6b13ec50160d63aca04f689e5cac9dd}{}\label{classsession_af6b13ec50160d63aca04f689e5cac9dd}


Additional tasks required to accomplish during the installation. 



Definition at line 24 of file session.\+class.\+php.

\index{session@{session}!module\+Update@{module\+Update}}
\index{module\+Update@{module\+Update}!session@{session}}
\subsubsection[{\texorpdfstring{module\+Update()}{moduleUpdate()}}]{\setlength{\rightskip}{0pt plus 5cm}session\+::module\+Update (
\begin{DoxyParamCaption}
{}
\end{DoxyParamCaption}
)}\hypertarget{classsession_ad1eb77a85d7765620658071e49d16cff}{}\label{classsession_ad1eb77a85d7765620658071e49d16cff}


Execute update. 



Definition at line 47 of file session.\+class.\+php.

\index{session@{session}!recompile\+Cache@{recompile\+Cache}}
\index{recompile\+Cache@{recompile\+Cache}!session@{session}}
\subsubsection[{\texorpdfstring{recompile\+Cache()}{recompileCache()}}]{\setlength{\rightskip}{0pt plus 5cm}session\+::recompile\+Cache (
\begin{DoxyParamCaption}
{}
\end{DoxyParamCaption}
)}\hypertarget{classsession_a08f5190f8867dc2c0d5c23435b236f15}{}\label{classsession_a08f5190f8867dc2c0d5c23435b236f15}


Re-\/generate the cache file. 



Definition at line 84 of file session.\+class.\+php.

\index{session@{session}!serialize\+Session@{serialize\+Session}}
\index{serialize\+Session@{serialize\+Session}!session@{session}}
\subsubsection[{\texorpdfstring{serialize\+Session(\$data)}{serializeSession($data)}}]{\setlength{\rightskip}{0pt plus 5cm}session\+::serialize\+Session (
\begin{DoxyParamCaption}
\item[{}]{\$data}
\end{DoxyParamCaption}
)}\hypertarget{classsession_aea7dba13f8bc60c916840c66b2bc28ba}{}\label{classsession_aea7dba13f8bc60c916840c66b2bc28ba}


session string encode 



Definition at line 72 of file session.\+class.\+php.

\index{session@{session}!session@{session}}
\index{session@{session}!session@{session}}
\subsubsection[{\texorpdfstring{session()}{session()}}]{\setlength{\rightskip}{0pt plus 5cm}session\+::session (
\begin{DoxyParamCaption}
{}
\end{DoxyParamCaption}
)}\hypertarget{classsession_a96fce5e6e2f2f0504cb1b9b4906ba379}{}\label{classsession_a96fce5e6e2f2f0504cb1b9b4906ba379}


Definition at line 16 of file session.\+class.\+php.

\index{session@{session}!un\+Serialize\+Session@{un\+Serialize\+Session}}
\index{un\+Serialize\+Session@{un\+Serialize\+Session}!session@{session}}
\subsubsection[{\texorpdfstring{un\+Serialize\+Session(\$val)}{unSerializeSession($val)}}]{\setlength{\rightskip}{0pt plus 5cm}session\+::un\+Serialize\+Session (
\begin{DoxyParamCaption}
\item[{}]{\$val}
\end{DoxyParamCaption}
)}\hypertarget{classsession_a97462709868f87d6cb050956c7b7da6d}{}\label{classsession_a97462709868f87d6cb050956c7b7da6d}


session string decode 



Definition at line 62 of file session.\+class.\+php.



\subsection{Member Data Documentation}
\index{session@{session}!\$lifetime@{\$lifetime}}
\index{\$lifetime@{\$lifetime}!session@{session}}
\subsubsection[{\texorpdfstring{\$lifetime}{$lifetime}}]{\setlength{\rightskip}{0pt plus 5cm}session\+::\$lifetime = 18000}\hypertarget{classsession_af73b8d0d1528237504be7b85f05c97ed}{}\label{classsession_af73b8d0d1528237504be7b85f05c97ed}


Definition at line 13 of file session.\+class.\+php.

\index{session@{session}!\$session\+\_\+started@{\$session\+\_\+started}}
\index{\$session\+\_\+started@{\$session\+\_\+started}!session@{session}}
\subsubsection[{\texorpdfstring{\$session\+\_\+started}{$session_started}}]{\setlength{\rightskip}{0pt plus 5cm}session\+::\$session\+\_\+started = false}\hypertarget{classsession_aae3391920555707a71e14e15f2929825}{}\label{classsession_aae3391920555707a71e14e15f2929825}


Definition at line 14 of file session.\+class.\+php.



The documentation for this class was generated from the following file\+:\begin{DoxyCompactItemize}
\item 
modules/session/\hyperlink{session_8class_8php}{session.\+class.\+php}\end{DoxyCompactItemize}
